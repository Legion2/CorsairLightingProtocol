...
void SerialLEDController::handleLEDControl(const Command& command, const CorsairLightingProtocolResponse* response)
{
...
void SerialLEDController::handleCallback()
{
	...
	if (readAvailable > 0) {
		unsigned int read = min(sizeof(buff) - part, (unsigned int) readAvailable);
		...
		if (part == sizeof(buff)) {
			if (buff[0] == PROTOCOL_RESPONSE_ERROR) {
				...
				this->response->send(&buff[1], sizeof(buff) - 1);
			}
			part = 0;
		}
	}
